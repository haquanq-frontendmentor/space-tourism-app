    ...
    <main className="flex grow flex-col">
      <Container className="flex flex-col">
        <div className="absolute inset-0 z-10 h-full *:h-full *:w-full *:object-cover">
          <Image className="md:hidden" src={MobileBackgroundImage} alt="" />
          <Image className="hidden md:block lg:hidden" src={TabletBackgroundImage} alt="" />
          <Image className="hidden lg:block" src={DesktopBackgroundImage} alt="" />
        </div>
        <section className="relative z-20 flex h-full grow py-[clamp(1.5rem,-4.7023rem+26.4631vw,8rem)] lg:items-end">
          <div className="flex w-full flex-col lg:flex-row lg:justify-between">
            ...
            <div className="flex items-center justify-center pt-[clamp(2.625rem,9.0983rem+-13.486vw,5.9375rem)] lg:p-0">
              <Link
                className="text-400 font-accent relative flex aspect-square w-[clamp(9rem,1.3664rem+32.57vw,17rem)] items-center justify-center rounded-full bg-white uppercase"
                href="/destination"
              >
                <span className="absolute inset-[clamp(3rem,0.6145rem+10.1781vw,5.5rem)] block rounded-full bg-white opacity-25"></span>
                Explore
              </Link>
            </div>
          </div>
